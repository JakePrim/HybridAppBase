...
public class DefaultNetDataPromptCover extends BaseCover {
    ...
    public DefaultNetDataPromptCover(Context context, boolean isSmall) {
        this(context);
        this.isSmall = isSmall;
        if (isSmall) {
            tv_content.setText("正在使用\n非Wi-Fi网络");
        }
    }
    ...
    public void onCoverNativeEvent(int eventCode, Bundle bundle) {
        ...
        switch (eventCode) {
            case CoverEventCode.COVER_EVENT_SMALL_TYPE:
                tv_content.setText("正在使用\n非Wi-Fi网络");
                break;
            case CoverEventCode.COVER_EVENT_COMMON_TYPE:
    ...
    private void notifyUI(int state) {
        if (state == -1) {
            ...
            if (!PrimPlayerCC.ALLOW_NETDATA_PLAY) {
                ...
                sendCoverEvent(COVER_EVENT_NET_CHANGE, null);
            } else {
