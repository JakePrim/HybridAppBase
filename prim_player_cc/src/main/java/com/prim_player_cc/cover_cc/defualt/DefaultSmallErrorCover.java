 ...
 */
public class DefaultSmallErrorCover extends BaseCover {
    ...
    public DefaultSmallErrorCover(Context context, boolean isError, boolean isSmall) {
        super(context);
        if (isError) {
        ...
        if (isSmall) {
            if (tv_video_error_cover != null) {
                tv_video_error_cover.setText("加载失败\n点击刷新");
            }
        }
        ll_live_video_error_cover = findViewById(R.id.ll_live_video_error_cover);
        if (ll_live_video_error_cover != null) {
    ...
    protected View createCoverView(Context context) {
        return View.inflate(context, R.layout.default_small_cover_error_layout, null);
    }
