 ...
 * 也可以通过{@link #sendCoverEvent(int, Bundle)} 桥接方法控制mediaplayer 等播放暂停等事件，还可以通知到其他视图做相应的处理
 * 更加灵活控制
...
public abstract class BaseCover implements ICover, ICoverOperate, View.OnClickListener {
    ...
    public void coverRequestPause() {
        ...
        sendCoverEvent(CoverEventCode.COVER_EVENT_PAUSE, null);
    }

    @Override
    public void coverRequestSeek(Bundle bundle) {
        PrimLog.d(TAG, "coverRequestPause 视图请求 decoder 跳转到指定位置");
        sendCoverEvent(CoverEventCode.COVER_EVENT_SEEK, bundle);
    }

    @Override
    public void coverRequestStart() {
        PrimLog.d(TAG, "coverRequestPause 视图请求 decoder 视频开始播放");
        sendCoverEvent(CoverEventCode.COVER_EVENT_START, null);
    }

    @Override
    public void coverRequestStop() {
        PrimLog.d(TAG, "coverRequestPause 视图请求 decoder 停止播放");
        sendCoverEvent(CoverEventCode.COVER_EVENT_STOP, null);
    }

    //通知我要变成全屏了
    @Override
    public void coverRequestFullScreen() {
        sendCoverEvent(CoverEventCode.COVER_EVENT_FULL_SCREEN, null);
    }

    //通知我要变成竖屏全屏
    @Override
    public void coverRequestVerticalFullScreen() {
        sendCoverEvent(CoverEventCode.COVER_EVENT_VERTICAL_FULL_SCREEN, null);
    }

    //通知我要变成竖屏了
    @Override
    public void coverRequestVerticalScreen() {
        sendCoverEvent(CoverEventCode.COVER_EVENT_VERTICAL_SCREEN, null);
    }
     ...
     * @param bundle
     */
    public void sendCoverEvent(int code, Bundle bundle) {
        if (onCoverEventListener != null && onCoverEventListener.get() != null) {
