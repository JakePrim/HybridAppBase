...
public class AssistPlayer implements IAssistPlay {
    ...
    public void setAspectRatio(int aspectRatio) {
        mPlayerView.setDiaplayAspectRatio(aspectRatio);
    }

    public int getCurrentAspectRatio() {
        return mPlayerView.getCurrentAspectRatio();
    }
...
//        senderCover(PRIM_PLAYER_EVENT_DISPOSABLE, null);
    ...
    public AssistPlayer setMute(boolean isMute) {
        if (isMute) {
    ...
    public AssistPlayer setTheme(@ColorInt int color) {
//        getPlayerView().getBusPlayerView().setBackgroundColor(color);
        return this;
    }
    ...
    public void resumePlay(@NonNull ViewGroup container) {
        resumePlay(container, null, false, true);
    }

    public void resumePlayVoice(@NonNull ViewGroup container, boolean isVoice) {
        resumePlay(container, null, false, isVoice);
    }

    public void resumePlay(@NonNull ViewGroup container, boolean addBottom) {
        resumePlay(container, null, addBottom, true);
    }

    public void resumePlay(@NonNull ViewGroup container, @NonNull PlayerSource source) {
        resumePlay(container, source, false, true);
    }

    public void resumePlay(@NonNull ViewGroup container, @NonNull PlayerSource source, boolean isVoice) {
        resumePlay(container, source, false, isVoice);
    }

    public void resumePlay(@NonNull ViewGroup container, PlayerSource source, boolean addBottom, boolean isVoice) {
        //当前的播放资源等于传入的播放资源
        if (source != null) {
        ...
        if (source != null) {
            if (TextUtils.isEmpty(source.getUrl())) {
                ...
                //如果是false isVoice 不受，则全局声音的控制，需要自己调用{#link setMute}方法进行单独的控制
                if (isVoice) {
                    //全局控制的声音
                    setVoice(isVoice());
                }
    ...
    private void attachContainer(ViewGroup container, boolean addBottom) {
    ...
    public void pause() {
//        PrimLog.printStackTrace(TAG + "pause: ");
        proxyDecoderCC.pause();
    }

    public void stop() {
//        PrimLog.printStackTrace(TAG + "stop: ");
        senderPlayEvent(PRIM_PLAYER_EVENT_STOP, null);
        detachPlayerContainer();
        proxyDecoderCC.stop();
    }

    public boolean isVoice() {
        isVoice = sharedPreferences.getBoolean(PLAYER_VOICE, true);
        return isVoice;
    }
     ...
     * @param bundle 传递的值
     */
    public AssistPlayer senderPlayEvent(int code, Bundle bundle) {
        if (null != mPlayerView) {
     ...
     * @param bundle 传递的值
     */
    public AssistPlayer senderNativeCoverEvent(int code, Bundle bundle) {
        if (null != mPlayerView) {
            BusPlayerView busPlayerView = mPlayerView.getBusPlayerView();
    ...
    public void openVerticalScreen(Activity activity) {
        ...
        //通知视图已经变为竖屏了
        senderPlayEvent(PRIM_PLAYER_EVENT_FULL_VERTICAL, null);
    }
    ...
    public int[] getPlayerViewLocation() {
        ...
        mPlayerView.getLocationOnScreen(location);
        return location;
    }
    ...
    public void openQuitVerticalFullScreen(Activity activity) {
        ...
        int moveWidth = (width);
        int moveHeight = (mOriginHeight + detalH);
        ...
        senderPlayEvent(PRIM_PLAYER_EVENT_VERTICAL_FULL, null);
    }
    ...
    public void openVerticalFullScreen(Activity activity) {
        ...
        final int x = playerViewLocation[0]; // view距离 屏幕左边的距离（即x轴方向）
        final int y = playerViewLocation[1]; // view距离 屏幕顶边的距离（即y轴方向）
        ...
        animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator animation) {
                ...
                com.prim_player_cc.view.ViewAnimator.putOn(busPlayerView).translation((1 - value) * x, (1 - value) * y);
    ...
    public void exitVerticalFullScreen(Activity activity) {
        ...
        animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator animation) {
                float value = (float) animation.getAnimatedValue();
                com.prim_player_cc.view.ViewAnimator.putOn(busPlayerView).translation(value * x, value * y);
        ...
        animator.addListener(new AnimatorListenerAdapter() {
            ...
            public void onAnimationEnd(Animator animation) {
                ...
                FrameLayout.LayoutParams params = new FrameLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,
                        ViewGroup.LayoutParams.MATCH_PARENT);
                PrimLog.e(TAG, "openVerticalFullScreen -> moveWidth:" + busPlayerView.getWidth() + " moveHeight:" + busPlayerView.getHeight());
                mPlayerView.addView(busPlayerView, params);
    ...
    public void destroy() {
        PrimLog.printStackTrace(TAG + "destroy: ");
        detachListener();
